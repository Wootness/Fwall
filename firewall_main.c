...
{
    conn_table_entry_t* ftp_data_entry = NULL;
	...
	else if(command_type == MANIPULATION_CMD_FTP_DATA)
	{
		// Adding the expected TCP connection to the dynamic table
		ftp_data_entry = (conn_table_entry_t *)kcalloc(1, sizeof(conn_table_entry_t), GFP_ATOMIC);
		if(!ftp_data_entry){
			// Failed to allocate, we can't monitor handle this connection
			return -ENOMEM;
		}
		ftp_data_entry->src_ip = server_ip;
		ftp_data_entry->dst_ip = client_ip;
		ftp_data_entry->src_port = 0; // Unknown 
		ftp_data_entry->dst_port = client_port;
		ftp_data_entry->state = TCP_STATE_EXPECTING_INIT;
		push_ftp_entry(ftp_data_entry);
	}
	else
	{
...
static int __init hw4secws_init(void) {
	...
	printk(KERN_INFO "Starting to install hw4secws module...\n");
	...
	printk(KERN_INFO "hw4secws module installed!\n");
...
static void __exit hw4secws_exit(void) {
	...
	printk(KERN_INFO "hw4secws module removed!\n");
}
