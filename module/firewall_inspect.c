...
int connection_table_inspect(struct sk_buff *skb,struct iphdr *ip_header, struct tcphdr* tcp_header,int direction, int only_append_existing)
{
    ...
    if(tcp_header->syn && !tcp_header->ack) {
        ...
        if(only_append_existing) {
            /* At this version of the FW, we do not allow only-appending packets*/
            return NF_DROP;
        }
        else {
            ...
            if(manipulate_preroute(skb,ip_header,tcp_header,current_dir_entry,direction) == MANIP_RESULTS_MANIPULATED && !tcp_header->ack){
    ...
    if(opposite_dir_entry && opposite_dir_entry->manipulation_port == tcp_header->dest 
        ...
        manipulate_preroute(skb,ip_header,tcp_header,opposite_dir_entry,direction);
        return NF_ACCEPT;
    }
    ...
    manipulate_preroute(skb,ip_header,tcp_header,current_dir_entry,direction);
    ...
    int (*okfn)(struct sk_buff *))
{
	...
	if(proto == PROT_TCP) {
        ...
        if(tcp_header->ack)
        {
            ...
            return connection_table_inspect(skb,ip_header,tcp_header,dir,1);
        }
        else if(tcp_header->syn)
        {
            ...
            if(connection_table_inspect(skb,ip_header,tcp_header,dir,1) == NF_ACCEPT)
            {
                return NF_ACCEPT;
            }
        }
	...
	for(i = 0 ;i<active_rules_count;i++) {
		...
		if(rule_match == RULE_CHECK_MATCHED){
            ...
            {
                // Updating dynamic rules table
                return connection_table_inspect(skb,ip_header,tcp_header,dir,0);
            }
