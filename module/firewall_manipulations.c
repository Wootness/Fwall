...
{
    if((tcp_header->source == HTTP_PORT || tcp_header->source == SMTP_PORT)
         && tcp_header->dest == matching_rule->manipulation_port) {
        // This is traffic from the server towards the client with OUR FW's PORT
        // re-routing packet to us - changing IP based on the interface we recieved it
    ...
    if(tcp_header->dest == HTTP_PORT || tcp_header->dest == SMTP_PORT) {
        ...
        else if(tcp_header->dest == SMTP_PORT)
            tcp_header->dest = SMTP_MANIPULATION_PORT;
        fix_checksums(skb,ip_header,tcp_header);
        return MANIP_RESULTS_MANIPULATED;
    }
...
int manipulate_output(struct sk_buff *skb,struct iphdr *ip_header, struct tcphdr* tcp_header)
{
    ...
    if(tcp_header->source == HTTP_MANIPULATION_PORT || tcp_header->source == SMTP_MANIPULATION_PORT) {
